...
This fork adds an optional intelligence layer that replaces Pagergotchi's
sequential attack logic with adaptive algorithms. Enable it in `config.conf`:
...
|------|----------|----------|
| **Active** | Balanced exploitation and exploration. Attacks high-value targets, explores new channels. | General wardriving |
| **Passive** | No active attacks. Focuses on channels with high client activity for natural handshake capture. | Stealth operation |
| **Assist** | Maximum aggression. Broadcast deauths, rapid channel hopping, broadest coverage. | Time-limited assessments |
...
optimize_timing = true       # Enable Bayesian timing optimization
...
Compared to stock Pagergotchi behavior in a simulated 50-AP environment over 100 epochs:
...
└── pwnagotchi_port/        # Main Python module
    ...
    ├── nextgen/            # NextGen intelligence layer (opt-in)
    │   ├── __init__.py     # NextGenBrain orchestrator
    │   ├── channel_bandit.py    # Thompson Sampling channel selection
    │   ├── tactical_engine.py   # Target scoring & attack routing
    │   ├── bayesian_optimizer.py  # GP-based timing optimization
    │   └── pineapd_adapter.py   # pineapd API translation layer
    └── ui/
