...
class StartupMenu:
    ...
    def __init__(self, config, display=None):
        ...
        # NextGen mode (active/passive/assist)
        self._nextgen_modes = ['active', 'passive', 'assist']
        self.nextgen_mode = self.settings.get('nextgen_mode',
                            config.get('nextgen', {}).get('mode', 'active'))
        if self.nextgen_mode not in self._nextgen_modes:
            self.nextgen_mode = 'active'
    ...
    def _cycle_nextgen_mode(self):
        """Cycle through NextGen modes: active -> passive -> assist -> active"""
        idx = self._nextgen_modes.index(self.nextgen_mode)
        self.nextgen_mode = self._nextgen_modes[(idx + 1) % len(self._nextgen_modes)]
    ...
    def _save_toggle_settings(self):
        ...
        self.settings['nextgen_mode'] = self.nextgen_mode
        save_settings(self.settings)
    ...
    def _draw_main_menu(self, selected, options):
        ...
        for i, opt in enumerate(options):
            ...
            is_toggle = is_toggle or opt.startswith('Mode:')

            if is_toggle:
                # Handle toggle options - use fixed label position so it doesn't shift
                if opt.startswith('Mode:'):
                    label = "Mode:"
                    value = self.nextgen_mode.upper()
                    value_color = theme['on'] if self.nextgen_mode == 'active' else theme['accent']
                elif opt.startswith('Privacy:'):
                    label = "Privacy:"
    ...
    def show_main_menu(self):
        ...
        while True:
            ...
            if btn == 'UP':
                ...
                # Handle toggles
                if selected == 1:  # Mode cycle
                    self._cycle_nextgen_mode()
                ...
                if selected == 0:  # Start
                    return True
                elif selected == 1:  # Mode cycle
                    self._cycle_nextgen_mode()
                    self._save_toggle_settings()
                    self._draw_main_menu(selected, options)
                elif selected == 2:  # Deauth Scope
