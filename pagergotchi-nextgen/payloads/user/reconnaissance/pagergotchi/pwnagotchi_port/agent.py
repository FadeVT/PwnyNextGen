...
NextGen Intelligence Integration:
- NextGenBrain initialized when [nextgen] enabled = true in config
- Handshake events forwarded to NextGen for skip logic
- NextGen state persisted alongside recovery data
"""
import time
import json
import os
...
class Agent(Client, Automata, AsyncAdvertiser):
    def __init__(self, view, config, keypair=None):
        ...
        self._nextgen = None
        nextgen_cfg = config.get('nextgen', {})
        if nextgen_cfg.get('enabled', False):
            try:
                from pwnagotchi_port.nextgen import NextGenBrain
                self._nextgen = NextGenBrain(config, self)
                logging.info("[NextGen] Intelligence layer initialized (mode=%s)", self._nextgen.mode)
            except Exception as e:
                logging.error("[NextGen] Failed to initialize: %s", e)
                self._nextgen = None
        else:
            logging.info("[NextGen] Disabled in config -- using stock behavior")
    ...
    def _save_recovery_data(self):
        ...
        # Also save NextGen state
        if self._nextgen:
            try:
                self._nextgen._save_state()
            except Exception as e:
                logging.error("Failed to save NextGen state: %s", e)
    ...
    async def _on_event(self, msg):
        ...
        if jmsg.get('tag') == 'wifi.client.handshake':
            ...
            if key not in self._handshakes:
                ...
                # Notify NextGen intelligence of handshake capture
                if self._nextgen:
                    channel = ap.get('channel', 0) if ap_and_station else 0
    ...
    def _should_interact(self, who):
        # NextGen skip logic: check capture context first (most impactful optimization)
        if self._nextgen and self._nextgen.has_handshake(who):
            return False
